...
const Doctors = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/doctors', {
        ...
        bio: formData.licenseNumber,
        isAvailable: formData.isAvailable
      });

      if (response.data?.success) {
        // ✅ إضافة الطبيب الجديد للقائمة
  ...
  // 🔍 تصفية البحث (باستخدام الحقول الصحيحة من الباكند)
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredDoctors.map((doctor, index) => (
                    <tr key={doctor.id} className="hover:bg-gray-50">
                      ...
                      {/* ✅ عرض الحقل الصحيح: name بدلاً من firstName+lastName */}
                      <td className="px-4 py-3 text-sm font-medium text-gray-800">{doctor.name}</td>
                      {/* ✅ عرض الحقل الصحيح: specialty بدلاً من specialization */}
...
// ✅ هذا السطر كان ناقصاً ويسبب فشل البناء!
